...
class CreateComment(CanCreateMixin, BaseCommentView):
    ...
    def form_valid(self, form):
        ...
        if user_email:
            email = EmailMessage(
                "نظر دریافت شد",
                "نظر شما دریافت شد. و به زودی به آن پسخ می دهیم.",
                to=[user_email]
            )
            email.send()
        ...
        if parent_email:
            email = EmailMessage(
                "پاسخ به نظر شما",
                ":پاسخی به نظر شما در مقاله {} ثبت شده است. برای مشاهده آن بر روی لینک زیر کلیک نماید\n{}{}".format(article, current_site, reverse('blog:detail', kwargs={'slug': article.slug})),
                to=[parent_email]
            )
            email.send()
